...
# Configure Streamlit
st.set_page_config(page_title="AI Crypto Forecast Dashboard",
                   layout="wide",
                   page_icon="📈")
...
def train_model(coin,
    ...
    try:
        ...
        scaler_path = os.path.join(SCALER_DIR,
                                   f"{coin}_{interval}_scaler.save")
        ...
        model_path = os.path.join(MODEL_DIR,
                                  f"{coin}_{model_type}_{interval}.keras")
...
def predict_future(model, last_seq, steps_pred, model_type, scaler):
    ...
    try:
        ...
        for _ in range(steps_pred):
            if model_type == "MLP":
                pred_scaled = model.predict(input_seq.reshape(1, -1),
                                            verbose=0)[0][0]
                input_seq = np.roll(input_seq, -1)
                input_seq[-1] = pred_scaled
            elif model_type == "CNN-LSTM":
                pred_scaled = model.predict(input_seq[np.newaxis, ...],
                                            verbose=0)[0][0]
...
def create_prediction_plot(historical_df, future_dates, predicted_prices,
                           title):
...
def main():
    ...
    with st.sidebar:
        ...
        if price is not None:
            price_placeholder.metric(
                label=
                f"Current {selected_coin_for_live.split('(')[0].strip()} Price",
                value=f"${price:,.4f}",
                delta=
                f"{price - fetch_live_price(COINS[selected_coin_for_live]) or 0:.4f}"  # Simple price change
            )
        else:
    ...
    with tab_train:
        ...
        with col3:
            neurons = st.slider("Neurons per Layer",
                                min_value=10,
                                max_value=200,
                                value=50,
                                step=10,
        ...
        if st.button("🚀 Train Models", use_container_width=True):
            for coin in coins_selected:
                with st.spinner(
                        f"Training {model_display_name} for {coin}..."):
                    result = train_model(coin, model_type_code, interval_train,
                                         epochs, batch_size, neurons, dropout,
                                         learning_rate)
